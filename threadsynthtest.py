...
class playNotes(Thread):
    def __init__(self, msg, execute_lock):
    ...
    def run(self):
        ...
        if hasattr(self.msg, 'note'):
            if self.msg.type == 'note_on':

                fs.noteon(0, self.msg.note, self.msg.velocity)#msg.velocity)
                last_velo = self.msg.velocity
                self.execute_lock.acquire()
                notearray.append(self.msg.note)
                self.execute_lock.release()
            elif self.msg.type == 'note_off':
                fs.noteoff(0, self.msg.note)
                self.execute_lock.acquire()
                notearray.remove(self.msg.note)
                self.execute_lock.release()

class checkControls(Thread):
    def __init__(self, msg, synth, execute_lock):
        Thread.__init__(self)
        self.msg = msg
        self.synth = synth
        self.daemon = True
    ...
    def run(self):
        ...
        if hasattr(self.msg, 'control'):

            faderHasChanged = False

            if self.msg.type == 'control_change':

                if self.msg.control == 74: #73
                    tmp[0,0] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 71: #75
                    tmp[0,1] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 76: #79
                    tmp[0,2] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 77: #72
                    tmp[0,3] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 18: #80:
                    tmp[0,4] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 19: #81:
                    tmp[0,5] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 16: #82:
                    tmp[0,6] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                elif self.msg.control == 17: #83:
                    tmp[0,7] = offset + self.msg.value/div_fac
                    faderHasChanged = True

                # TODO: map 9th fader to lowpass filter cutoff freq

                elif self.msg.control == 85:
                    # lowpass filter with cutoff freq,

                    lowpass_filter_value = 2*(2000 - self.msg.value*10)
                ...
                if faderHasChanged == True:
                    ...
                    filename_= '60'
                    self.synth.execute(tmp,filename_)
...
filename_= '60'
synth.execute(tmp,filename_,True)
