...
class ANNeSynth:
	...
	def execute(self,values,filename='long',doFirstThing = False):
                #print(values)
		...
		#Prints validation accuracy of the trained ANN
                ...
                if self._operationMode.control:
                        len_window = 256#128#512#1024#2048#4096 #Specified length of analysis window
                        hop_length = len_window#1#128#256#512#1024 #Specified percentage hop length between windows
                        t = time()
                        n_frames = 10#20#50#100
                        ...
                        # following two lines take care of frequency shift
                        t1 = time()
                        #for ii in range(n_frames):
                        ...
                        #Note: Do we REALLY need this first .wav file? We're only using _raw.wav file since that generates the actual .wav file.
                        #If not we can save another write
                        
                        #sf.write(filename+'.wav', U, 44100, subtype='PCM_16')  #Must be 16bit PCM to work with pygame
                        sf.write(filename+'_raw.wav', U, 44100, subtype='PCM_16')  #Must be 16bit PCM to work with pygame

                        tfinal = time()
                        print('Method took '+str(tfinal - t)+' seconds to process the whole file')
                        print('for loop: ' + str(t1-t))
                        print('initialize phase: ' + str(t2 - t1))
                        print('normalize: ' + str(t3-t2))
                        print('sf.write: ' + str(tfinal-t3))
	...
	def play_synth(self,values):
		...
		n_frames = 200 #determines audio file durations -> decrease to increase mem and perf
		mag_buffer = np.zeros((self.topology.output_size,1))
		activations = values[:,0:8]
		for ii in range(n_frames):
			orig_hat = np.reshape(self._sess.run(self.outputLayer2,feed_dict={self.controller:activations}),[self.topology.output_size,-1])
